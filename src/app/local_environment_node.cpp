...
ros::Publisher local_grid_map_pub;
void publishCurrent_gridMap(nav_msgs::OccupancyGrid &grid_map_msg){
         if (local_grid_map_pub.getNumSubscribers() == 0)
        {
            return;
        }
            local_grid_map_pub.publish(grid_map_msg);
}
...
int main(int argc, char** argv)
{
  ...
  //  Pointcloud3dProcess *pointcloud_3d_process_ptr = new Pointcloud3dProcess(config);
   ...
   std::shared_ptr<CloudPublisher> local_cloud_pub_ptr = std::make_shared<CloudPublisher>(nh, "pcl_output",pointcloud_2d_process_ptr->topic_frame_id, 100);
   std::shared_ptr<CloudPublisher> local_3d_grid_pub_ptr = std::make_shared<CloudPublisher>(nh, "inflation_map",pointcloud_2d_process_ptr->topic_frame_id, 100);

   //这里创建发布二维占据栅格地图对象并没有封装为类
   local_grid_map_pub = nh.advertise<nav_msgs::OccupancyGrid>("/grid_map_realtime", 100);
   ...
   while(ros::ok())
   {
     ...
     if(cloud_data_buff.size() > 0)
     {
       ...
       //发布实时三维点云
       pointcloud_2d_process_ptr->find_Z_value(cloud_data.cloud_ptr);
       pointcloud_2d_process_ptr->PassThroughFilter(cloud_data.cloud_ptr,cloud_data,false,pointcloud_2d_process_ptr->current_max_z_adjust,pointcloud_2d_process_ptr->current_min_z_adjust);
       local_cloud_pub_ptr->Publish(cloud_data.cloud_ptr);

       //发布实时二维栅格
       pointcloud_2d_process_ptr->Pointcloud_to_2d_grid(cloud_data,local_grid_map_msg,pointcloud_2d_process_ptr->current_map_resolution);
       local_grid_map_msg.header.frame_id = pointcloud_2d_process_ptr->topic_frame_id;
       publishCurrent_gridMap(local_grid_map_msg);
       //发布实时三维栅格
